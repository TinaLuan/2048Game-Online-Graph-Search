...
int main(int argc, char *argv[]) {
	...
	// initialize output
	int num_generated_node = 0;
	int num_expanded_node = 0;
	clock_t start = clock();
	...
	while (true) {
		 ...
		 */
		move_t selected_move = get_next_move( board, max_depth, propagation, &num_generated_node, &num_expanded_node);
		/**
	...
	fprintf(output, "Max Depth = %d\n", max_depth);
	fprintf(output, "Generated = %d\n", num_generated_node);
	fprintf(output, "Expanded = %d\n", num_expanded_node);
	fprintf(output, "Time = %.2f seconds\n", time);
	fprintf(output, "Expanded/Second = %.0f\n", num_expanded_node / time);
	fprintf(output, "Max Tile = %d\n", find_max_tile(board));
